...
@REM docker exec -it %CONTAINER_NAME% sh -c "ansible-playbook -i %INVENTORY% -b %DOCKER_PLAYBOOK%"
@REM docker exec -it %CONTAINER_NAME% sh -c "ansible-playbook -i %INVENTORY% -b %ACTION_PLAYBOOK%"
docker exec -it ansible sh -c "ansible-playbook -i inventory/hosts/hosts.yml playbook.yml --tags runner"
